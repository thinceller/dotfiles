...
in
{
  ...
  programs.claude-code = {
    ...
    settings =
      ...
      // lib.optionalAttrs isPersonal {
        # auto mode classifier の設定。classifier は user / managed settings しか読まず、
        # リポの .claude/settings.json は無視される (repo 側から allow を注入させない
        # ため)。ここには全プロジェクト共通の一般論だけを書き、リポ固有の文脈
        # (公開範囲・deploy 経路・secrets の扱い) は各リポの CLAUDE.md に書く
        # (classifier は CLAUDE.md も読む)。
        # "$defaults" を省くとそのリストの組み込み定義ごと置き換わるので必ず入れる。
        # 仕事機は org 情報が別なので、そちらで /auto-mode-setup を実行して設定する。
        autoMode = {
          environment = [
            "$defaults"
            "**Organization**: None — personal GitHub account `thinceller`; no employer resources are reachable from these machines"
            "**Primary use of Claude Code**: software development on personal projects (Nix dotfiles, personal repos, OSS contributions)"
            "**Host containment**: None — Claude Code runs directly on personal Macs (kohei-m4-mac-mini) and a personal NixOS server (oberon) with open internet"
            "**Secrets management**: SOPS with age; the age private key at ~/.config/sops/age/keys.txt must never be read, printed, or transmitted, while secrets/*.yaml files in repos are ciphertext and safe to read or commit"
          ];
          allow = [
            "$defaults"
            "Bash(nix fmt)"
