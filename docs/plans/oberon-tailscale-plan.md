...
> **Status**: 実装済み (commit `bc9ee6d` / 2026-06-24)。`ssh oberon` は Tailscale MagicDNS 経由、
> `ssh oberon-cf` が cloudflared 経由の fallback。本ドキュメントは設計・手順の経緯記録として残す。
...
> Tailscale / deploy-rs) のうち「本命」に当たる。deploy 方式は [`oberon-deploy.md`](../reference/oberon-deploy.md) 参照。
...
([`oberon-deploy.md`](../reference/oberon-deploy.md) 方式 A 相当)。**既存の cloudflared 経路をそのまま使って 1 回 deploy**
...
- 最終 fallback: Sakura VNC コンソール ([`sakura-vps-nixos-lessons.md`](../reference/sakura-vps-nixos-lessons.md) §14)
...
- [x] Tailscale 管理コンソールで tag:server / ssh ACL を定義
- [x] tag:server 付き auth key を発行
- [x] `sops secrets/oberon.yaml` に `tailscale-authkey` を登録
- [x] `hosts/oberon/tailscale.nix` 作成
- [x] `hosts/oberon/configuration.nix` の imports に追加
- [x] `nix-darwin/modules/homebrew.nix` に `tailscale` cask 追加
- [x] `home-manager/programs/ssh/default.nix` の `oberon` / `oberon-cf` 整理
- [x] `git add` (新規ファイルは flake が見えるよう必須) → commit → push
- [x] oberon を cloudflared 経由 (tmux 内) で `nixos-rebuild switch`
