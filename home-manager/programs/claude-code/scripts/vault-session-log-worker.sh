...
$VAULT/$notefile にセッションログを書いてください (このパス以外には書かない。
Write/Edit にはこの絶対パスをそのまま渡す)。
既にファイルが存在する場合は、既存の内容も踏まえて全体を書き直して更新してください。
その際 frontmatter の created は既存の値を維持してください (更新時刻で書き換えない)。
...
ルール:
- 書いてよいのは $VAULT/$notefile のみ。他のファイル (Notes/ 等) には触らない
